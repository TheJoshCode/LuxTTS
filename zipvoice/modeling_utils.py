...
def process_audio(audio, prompt_text, tokenizer, feature_extractor, device, target_rms=0.15, duration=4, feat_scale=0.1):
    """
    Process audio prompt using manually provided transcription text.
    ...
    Returns:
        tuple: (prompt_tokens, prompt_features_lens, prompt_features, prompt_rms)
    """
    # Validate prompt_text
    if not prompt_text or not isinstance(prompt_text, str):
        raise ValueError("prompt_text must be a non-empty string. Manual transcription is required.")
    
    # Load audio at required sample rates
    prompt_wav, sr = librosa.load(audio, sr=24000, duration=duration)
    
    # Log the manually provided transcription (no automatic transcription)
    print(f"Using manual transcription: '{prompt_text}'")
    ...
    # Tokenize the manually provided text
    prompt_tokens = tokenizer.texts_to_token_ids([prompt_text])
    
    return prompt_tokens, prompt_features_lens, prompt_features, prompt_rms


def generate(prompt_tokens, prompt_features_lens, prompt_features, prompt_rms, text, model, vocoder, tokenizer, 
             num_step=4, guidance_scale=3.0, speed=1.0, t_shift=0.5, target_rms=0.15):
    """
    Generate speech from text using encoded prompt features.
    """
    tokens = tokenizer.texts_to_token_ids([text])
    device = next(model.parameters()).device  # Auto-detect device

    # Apply speed multiplier (removed the 1.3x multiplier for consistency)
    speed = speed * 1.0
...
def load_models_gpu(model_path=None, device="cuda"):
    """
    Load models for GPU inference.
    Note: No transcriber is loaded - manual transcription is required.
    """
    params = LuxTTSConfig()
    ...
    # No automatic transcriber - manual transcription only
    transcriber = None
...
def load_models_cpu(model_path=None, num_thread=2):
    """
    Load models for CPU inference using ONNX.
    Note: No transcriber is loaded - manual transcription is required.
    """
    params = LuxTTSConfig()
    params.seed = 42
